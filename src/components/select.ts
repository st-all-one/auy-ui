...
export class AuyCompSelect extends DataAwareMixin(LitElement) {
  ...
  @property({ type: String }) placeholder = t('selectPlaceholder');
  ...
  override render() {
    ...
    return html`
      ...
      <div data-element="select" class="wrap">
        <div
          id=${this._selectId}
        ...
        <div class="dropdown ${this._open ? 'open' : ''}">
          ${this.searchable ? html`
            <input
              ...
              aria-label=${t('selectSearchLabel')}
            />
          ` : nothing}
          ...
          ` : html`
            <div class="no-results">${t('selectEmpty')}</div>
          `}
        </div>
      </div>
    `;
