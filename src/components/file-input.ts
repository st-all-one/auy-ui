    ...
    return html`
      ...
      >
          ...
          <div class="file-grid">
            ${this._files.map((f, i) => html`
              <div class="file-card">
                ...
                <button class="file-card-remove" @click=${(e: Event) => { e.stopPropagation(); this._removeFile(i); }} aria-label=${t('fileRemove', { name: f.name })}>&times;</button>
              </div>
            `)}
            ${this.multiple ? html`
              <div class="add-more">
        ...
        ` : html`
          ...
          <div class="empty-text">${t('fileDropzone')}</div>
        `}
        <div class="hint">${this.accept ? t('fileFormats', { accept: this.accept }) : ''}${this.maxSize && !this.accept ? t('fileMaxSize', { size: this._formatSize(this.maxSize) }) : ''}${this.maxSize && this.accept ? t('fileMaxSizeSuffix', { size: this._formatSize(this.maxSize) }) : ''}</div>
        <input
          type="file"
          ?multiple=${this.multiple}
          accept=${this.accept || nothing}
          @change=${this._handleChange}
          aria-label=${this.label || t('fileSelect')}
        />
      </div>
    `;
  }
