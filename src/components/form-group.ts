...
const VALIDATORS: Record<string, Validator> = {
  required: (v) => !v.trim() ? t('validationRequired') : null,
  cpf: (v) => {
    const d = stripMask(v);
    return d.length > 0 && !isValidCPF(d) ? t('validationCpf') : null;
  },
  cnpj: (v) => {
    const d = stripMask(v);
    return d.length > 0 && !isValidCNPJ(d) ? t('validationCnpj') : null;
  },
  email: (v) => !!v && !isValidEmail(v) ? t('validationEmail') : null,
  minLength: (v, p) => v.length < +(p ?? 0) ? t('validationMinLength', { min: p ?? '' }) : null,
  maxLength: (v, p) => v.length > +(p ?? 0) ? t('validationMaxLength', { max: p ?? '' }) : null,
  pattern: (v, p) => {
    if (!p || !v) return null;
    return new RegExp(p).test(v) ? null : t('validationPattern');
  },
  strongPassword: (v) => {
    if (!v) return null;
    const errors: string[] = [];
    if (v.length < 8) errors.push(t('validationPasswordMin'));
    if (!/[A-Z]/.test(v)) errors.push(t('validationPasswordUpper'));
    if (!/[a-z]/.test(v)) errors.push(t('validationPasswordLower'));
    if (!/\d/.test(v)) errors.push(t('validationPasswordDigit'));
    if (!/[^A-Za-z0-9]/.test(v)) errors.push(t('validationPasswordSpecial'));
    return errors.length ? t('validationPasswordPrefix') + errors.join(', ') : null;
  },
};
...
export class AuyCompFormGroup extends LitElement {
  ...
  override render() {
    ...
    return html`
      ...
      <div data-element="form-group" class="fg-wrap" ?inline=${this.inline}>
        ${this.label ? html`
